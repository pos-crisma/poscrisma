...
import '../action/match_talent.dart';
import '../state/match_talent.dart';

class MatchTalentReducer extends Reducer<MatchTalentAction, MatchTalentState> {
  MatchTalentReducer(BuildContext context)
      : super(MatchTalentState(context: context));

  @override
  Future<Effect> reduce(MatchTalentAction action) async {
    return action.when(
      onAppear: () {
        state.user = profileStore.user;

        return Effect.runAndEmit(() async {
          send(const MatchTalentAction.service());
          send(const MatchTalentAction.serviceGame());
        });
      },
      backButton: () {
      ...
      service: () {
        return Effect.run(() async {
          final result = await ScheduleAPI.getTalent();

          send(MatchTalentAction.update(result));
        });
      },
      update: (schedule) {
          ...
          if (state.data != null && state.data!.gameId != null) {
            ...
            resultStatus
                ? await GameAPI.findGameByID(gameId).fold(
                    (success) => send(MatchTalentAction.successGame(success)),
                    (_) => send(const MatchTalentAction.offline()),
                  )
                : send(const MatchTalentAction.offline());
          }
        });
      },
      successGame: (game) {
          ...
          if (state.data != null && state.data!.gameId != null) {
            ...
            try {
              if (value != null) {
                ...
                send(MatchTalentAction.successGame(game));
              }
            } catch (e) {
              ...
              send(MatchTalentAction.failure(error));
            }
          }
        });
      },
