...
import '../provider/action/match_talent.dart';
import '../provider/store/match_talent.dart';

class MatchTalent extends StatefulWidget {
  const MatchTalent({
    super.key,
  });

  @override
  State<MatchTalent> createState() => _MatchTalentState();
}

class _MatchTalentState extends State<MatchTalent> {
  late final MatchTalentReducer viewStore;

  @override
  void initState() {
    super.initState();

    viewStore = MatchTalentReducer(context);
    viewStore.send(const MatchTalentAction.onAppear());
  }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      canPop: false,
      onPopInvoked: (didPop) =>
          viewStore.send(const MatchTalentAction.backButton()),
      child: Scaffold(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ScoreMatch(
                            ...
                            minusButton: () => viewStore.send(
                              MatchTalentAction.minusTapped(
                                data.teamId ?? "",
                              ),
                            ),
                            plusButton: () => viewStore.send(
                              MatchTalentAction.addedTapped(
                                data.teamId ?? "",
                              ),
                            ),
                            score: data.score ?? 0,
